...
int poifs_getattr(const char* path, struct stat* stbuf) {
	...
	if (string(path) == "/"){
		...
		if (entry.getAttr() & 0x8) {
			stbuf->st_mode = S_IFDIR | (0005 + ((entry.getAttr() & 0x7) << 6) + ((entry.getAttr() & 0x7) << 3));
		}
		else {
			stbuf->st_mode = S_IFREG | (0004 + ((entry.getAttr() & 0x7) << 6) + ((entry.getAttr() & 0x7) << 3));
		}
